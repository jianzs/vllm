...
| 指标 | 配置 4<br>DyCP DP | 配置 1<br>全 CP | 配置 2<br>CP1+PD | **配置 3**<br>**DyCP** |
|------|-------------------|-----------------|-------------------|------------------------|
| 完成数 | 500 | 500 | 500 | **500** |
| **TTFT 均值** | 1,022 ms | 1,268,765 ms | 20,927 ms | **311 ms** |
| **TTFT P50** | 674 ms | 1,621,303 ms | 19,604 ms | **253 ms** |
| **TPOT 均值** | 24.56 ms | 54.71 ms | 32.79 ms | **13.51 ms** |
| **TPOT P50** | 25.95 ms | 55.06 ms | 36.56 ms | **13.58 ms** |
| **ITL 中位数** | 9.68 ms | — | — | **9.74 ms** |
| **ITL P99** | 203.13 ms | — | — | **131.02 ms** |
| **E2EL 均值** | 26,142 ms | 1,324,737 ms | 54,471 ms | **14,127 ms** |
...
|------|-------------------|------------------------|------|
| 完成数 | 500 | **500** | — |
| **TTFT 均值** | 24,944 ms | **10,165 ms** | **-59%** |
| **TPOT 均值** | 17.75 ms | **13.80 ms** | **-22%** |
| **ITL 中位数** | 9.63 ms | **10.32 ms** | +7% |
| **ITL P99** | 203.14 ms | **143.45 ms** | **-29%** |
| **E2EL 均值** | 61,279 ms | **38,418 ms** | **-37%** |
| **输出吞吐** | 3,308 tok/s | **3,697 tok/s** | **+12%** |
| **最大并发** | 150 | **100** | -33% |
...
| vs 配置 4（DyCP DP） | vs 配置 2（PD 无 CP） |
|---|---|
| TTFT: **-70%** (311ms vs 1,022ms) | TTFT: **-98.5%** (311ms vs 20,927ms) |
| TPOT: **-45%** (13.51ms vs 24.56ms) | TPOT: **-59%** (13.51ms vs 32.79ms) |
| E2EL: **-46%** (14.1s vs 26.1s) | E2EL: **-74%** (14.1s vs 54.5s) |
...
|------|-----------|-----------|---------|
| 配置 4 | 1,022 ms | 24,944 ms | **24.4×** |
| 配置 3 | 311 ms | 10,165 ms | **32.7×** |

根因不是 prefill 本身变慢，而是**更长输出导致 KV cache 占用时间更长，排队延迟增加**。配置 3 增长倍数更高是因为 8K/1K TTFT 基数极低（311ms），但绝对值仍远优于配置 4（10.2s vs 24.9s，快 2.4 倍）。
...
| 配置 4 (DyCP DP) | 2K | 9.63 ms | 203.14 ms | 21.1× |
| 配置 3 (DyCP) | 2K | 10.32 ms | 143.45 ms | 13.9× |
...
|------|-----------|-----------|------|
| 配置 4 | 24.56 ms | 17.75 ms | -28% |
| 配置 3 | 13.51 ms | 13.80 ms | +2% |
...
配置 3 的输出吞吐比配置 4 高 12%（3,697 vs 3,308 tok/s），E2EL 低 37%。
...
2. **配置 3（DyCP）是整体最优配置**：
   ...
   - 8K/2K 并发：TTFT -59%（10.2s vs 24.9s），TPOT -22%，E2EL -37%
